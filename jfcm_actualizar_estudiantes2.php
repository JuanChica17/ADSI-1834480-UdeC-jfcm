...
$sentencia2 = $bd->prepare("UPDATE estudiantes_jfcm SET nombre = ?, apellido = ?, email = ?, password = ?, num_doc = ?  WHERE id = ?;");
$resultado2 = $sentencia2->execute([$nombre,$apellido,$email,$password,$num_doc,$id]);

if($resultado2 === TRUE){
  echo "<div class='alert alert-info'>Datos Del Estudiante Actualizados Correctamente</div>";
  }else{
    
    echo "<div class='alert alert-warning'>Error Al Actualizar Los Datos Del Estudiante</div>";
  }
